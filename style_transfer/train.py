...
from loss import get_loss, gram, per_pixel_loss
import os
...
def train_model():
    ...
    # style_image_1 = Image.open("./style_image/ark2.jpeg")
    # style_image_1 = transform_style(style_image_1)
    # style_image_1 = style_image_1.to(device)


    # style_image_2 = Image.open("./style_image/ark3.jpeg")
    # style_image_2 = transform_style(style_image_2)
    # style_image_2 = style_image_2.to(device)

    # style_image_3 = Image.open("./style_image/ark4.jpeg")
    # style_image_3 = transform_style(style_image_3)
    # style_image_3 = style_image_3.to(device)

    # style_image_4 = Image.open("./style_image/ark5.jpeg")
    # style_image_4 = transform_style(style_image_4)
    # style_image_4 = style_image_4.to(device)

    # style_image = torch.stack((style_image, style_image_1, style_image_2, style_image_3, style_image_4))
    ...
    for epoch in range(1, NUM_EPOCHES + 1):
        ...
        for data, _ in tqdm(dataloader, desc=f"Epoch {epoch}", unit="batch"):
            ...
            # loss += 1e-7 * tv_loss
            loss = per_pixel_loss(y_hat, style_image) + per_pixel_loss(y_hat, data)
            loss.backward()
            optimizer.step()
            ...
            if iterations % SAMPLES_EVERY == 0:
                ...
                output_man = model(man).cpu()
                man_path = "visualization/eva/man/man_%d_%05d.jpg" %(epoch, batch_idx)
                transform_back(output_man, man_path)
